...
pub fn main() !void {
    ...
    while (!quit) {
        ...
        while (r.sdl.SDL_PollEvent(&event) != 0) {
            ...
            switch (game_menu.state) {
                ...
                .game_running => {
                    // Your existing event handling goes here
        ...
        // Your existing keyboard state handling (SHIFT+key combos)
    ...
    //         continue;
    //     } else {
    //         r.render(renderer, font, kr.moduliList[current_modulo].items[current_matrix], current_matrix, current_modulo) catch |err| {
    //             return err;
    //         };
    //         if (kr.number_of_calcmatrices < current_matrix + loading_constant) {
    //             try r.render_loading_matrices(renderer, font);
